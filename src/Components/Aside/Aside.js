...
import { Button, Col, Input, Icon, Row, SideNav } from 'react-materialize';
...
export class Aside extends Component {
 ...
 /*
   ...
   */
 filerLocationBySearch = (locations, searchBy) => locations.filter(location => {
  // RegExp to match the value if it contains in string (gi = global and case
  // insensitive)
 ...
 render() {
  ...
  const locations = !isHidden
   ? this.filerLocationBySearch(markers, search)
   : [];

  return (
   <SideNav
    trigger={<Button className = "hide" tabIndex = "-5" > SIDE NAV</Button>}
    id="wrapper-aside"
    options={{
     closeOnClick: true
    }}
    ...
    <h3 >Plan your perfect lake escape in
     <span id="bregenz">Bregenz
     </span><span><img src={atFlag} alt="Austrian flag"/></span>
    </h3>
    <h4 >Quick Filter</h4>
    <Row id="btns-filter">
     <Col s={4}>
      <Button
       ...
       onClick={() => {
        this
         .props
         .filterMarkers('filter-culture')
       }}>
       <Icon large="large">location_city</Icon>
      </Button>
     </Col>
     <Col s={4}>
      <Button
    ...
    <a
     ...
     aria-label="go to foursquare"
     className="foursquare-logo">
     Powered by
     <img src={foursquareIMG} className="img-responsive" alt="foursquare"/></a>
   </SideNav>
  );
 }
}
